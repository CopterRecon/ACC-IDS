...
class TwoCarSimulator:
    ...
    def __init__(
        ...
        cfg,
        init_gap_m,
        ids_accuracy        # <<< CHANGE: accuracy passed per run
    ):
        self.host = host
        self.lead = lead
        ...
        self.ids_accuracy = ids_accuracy  # <<< CHANGE: stored per simulation
        self.records = []
    ...
    def run(self, scenario):
        """
        Returns:
            crash_time (int): time index i where crash occurs
                              -1 if no crash
        """

        crash_time = -1     # <<< CHANGE: instantiated BEFORE loop
        ActivateIDS = 0
        ...
        for i in range(self.cfg.steps):
            ...
            # <<< CHANGE: noise explicitly added as requested
            ...
            # <<< CHANGE: Scenario-4 IDS enforcement
            if scenario == 4 and ActivateIDS:
                host_th = 0.0
                host_br = 1.0
                ActivateIDS = 0
            ...
            # <<< CHANGE: attack injected only for scenario ≥ 3
            if scenario >= 3:
                z_attack, attack_active, _ = self.attacker.apply(vH_real, i)

                # <<< CHANGE: IDS decision is probabilistic per run
            ...
            # <<< CHANGE: crash detection returns i immediately
            if self.gap_m < self.cfg.stop_gap_m:
                crash_time = i
                return crash_time   # <<< CHANGE: immediate return

            # ===== Logging (kept to preserve ALL plots) =====
            self.records.append({
                "step": i,
                "gap_m": self.gap_m,
                "d_safe": d_safe,
        ...
        return crash_time   # <<< CHANGE: -1 if no crash
